        ...
        <div className="profile">
            <div className="profile_card-profile">
                ...
                <div className="profile_card-inputs">
                    <Input title="Nome" name="nome" onChange={handleChange} disabled={disabled}
                           value={formValues.nome}/>
                    <Input title="E-mail" name="email" onChange={handleChange} disabled={disabled}
                           value={formValues.email}/>
                    <Input title="Universidade" name="universidade" onChange={handleChange} disabled={disabled}
                           value={formValues.universidade}/>
                    <div className="disciplinas">
                        <p>Disciplinas</p>
                        <div className="list-disciplinas">
                            {materias.map((materia, index) => (
                                    <div key={index} className="materia-box">{materia}</div>
                                )
                            )}
                        </div>
                    </div>
                </div>
            {disabled === '' ? <div onClick={deleteAccount} className="profile_card-delete-account">Excluir Conta</div> : null}
            </div>
        </div>
    )
}
